...
//    layout->addWidget(m_diskWrite, 1, 2);
    ...
    m_speedInfo = new SpeedInfo(this);
...
void SpeedWidget::onUpdateNet()
{
    ...
    m_speedInfo->netRate(down, upload);

    downRate = m_speedInfo->autoRateUnits(down - m_down, unit);
    QString downUnit = m_speedInfo->setRateUnitSensitive(unit, SpeedInfo::Default);
    unit = SpeedInfo::RateByte;
    uploadRate = m_speedInfo->autoRateUnits(upload - m_upload, unit);
    QString uploadUnit = m_speedInfo->setRateUnitSensitive(unit, SpeedInfo::Default);
